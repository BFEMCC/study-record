...
class webServer:
    ...
    def handling_requests(self, client: socket):
        ...
        if path.endswith(".html"):
            body = application(path)
            client.send(self.response + body)

        else:  # 如果不是html视为静态资源
            try:  # 读取资源文件
                with open("./static"f"{path}", "rb") as f:
                    body = f.read()
                client.send(self.response + body)

            except FileNotFoundError:  # 若没有指定文件,返回404
                body = application("/404NOT FUOND.html")
                client.send(self.error + body)
